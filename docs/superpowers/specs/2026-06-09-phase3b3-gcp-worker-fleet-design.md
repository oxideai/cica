...
- **Integration (gated, GCS has an emulator):** `GcsStateStore` round-trip (absent→false, push/pull byte-for-byte, replace semantics, a >resumable-threshold large file) against **`fake-gcs-server`** (Docker, HTTP, tokenless), gated by `CICA_GCS_IT` — the GCS analog of the `CICA_S3_IT`/LocalStack test. New CI `gcs-store` job paralleling `s3-store`.
...
- **CI build/lint lane:** `cargo clippy --features cloud --all-targets -- -D warnings` so both `gcs` and `cloudrun` code compiles + lints even though the launcher can't run.
