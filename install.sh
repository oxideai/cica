...
should_install() {
    ...
    if [ "$CICA_VERSION" = "latest" ]; then
        ...
        if command -v curl >/dev/null 2>&1; then
            LATEST_VERSION=$(curl -fsSL https://api.github.com/repos/oxiglade/cica/releases/latest | grep -o '"tag_name": *"v[^"]*"' | sed 's/"tag_name": *"v\([^"]*\)"/\1/' 2>/dev/null || echo "$CURRENT_VERSION")
        else
...
main() {
   ...
   │         assistant               │
   │ https://github.com/oxiglade/cica │
   └─────────────────────────────────┘
    "
